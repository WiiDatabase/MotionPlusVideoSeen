/*-------------------------------------------------------------
 ...
 Few modifications by iCON <wiidatabase.de> (c) 2019
...
u8 *__SYSCONF_Find(const char *name) {
    ...
    while(count--) {
        if((nlen == ((__sysconf_buffer[*offset]&0x0F)+1)) &&
                !memcmp(name, &__sysconf_buffer[*offset+1], nlen))
...
s32 SYSCONF_GetMovieSeen(void) {
    int res;
    u8 val = 0;
    res = SYSCONF_Get("MPLS.MOVIE", &val, 1);
    if(res<0) return res;
    if(res!=1) return SYSCONF_EBADVALUE;
    return val;
}
...
s32 SYSCONF_SetMovieSeen(u8 value) {
    return SYSCONF_Set("MPLS.MOVIE", &value, 1);
}
